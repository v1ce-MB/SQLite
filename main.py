...
GL_total_wins = pd.read_sql("SELECT COUNT(Match_Id) AS GLwins from Match WHERE Match_Winner == 13;", conn)
#print(GL_total_wins)
...
result = pd.read_sql("SELECT COUNT (Player_Id) AS Number_of_Players, Country_Name  from Player GROUP BY Country_Name;", conn)
#print(result)
...
result2 = pd.read_sql ("""
SELECT Match_Id, Match_Winner, Team_Name AS Match_winner
FROM match 
LEFT JOIN Team                     
...
SHMatches_2015 = pd.read_sql("""
SELECT Match_Id, Team_2 AS Away_Team, Match_Winner
FROM Match
WHERE Team_1 IN  
(SELECT Team_1 FROM Match
WHERE Team_1 == 11 AND Season_Id == 8 )
                             ;
""",conn)
#print(SHMatches_2015)

results5 = pd.read_sql("""
